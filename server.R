...
function(input, output) {
  ...
  output$plot4 <- renderPlot({
    ...
    plot4shiny + geom_point(aes(alpha = .5, color = Region, size = .1)) + guides(alpha = F, size = F) + xlab("DPT Immunization") + ylab("Life Expectancy (years)") + ggtitle("DPT Immunization and Life Expectancy")
    
  })
  output$plot5 <- renderPlot({
    plot5shiny = ggplot(data = wb.data2, aes(x = Imm.Meas, y = Life.E))
    plot5shiny + geom_point(aes(alpha = .5, color = Region, size = .1)) + guides(alpha = F, size = F) + xlab("DPT Immunization") + ylab("Life Expectancy (years)") + ggtitle("DPT Immunization and Life Expectancy")
  ...
  output$table1 <- renderTable(wb.data)
}
